...
class SOM(nn.Module):
    ...
    def __init__(self, m, n, dim, niter, alpha=None, sigma=None):
        ...
        self.weights = torch.randn(m * n, dim, device=torch.device("cuda"))
        self.locations = torch.cuda.LongTensor(np.array(list(self.neuron_locations())))
    ...
    def fit_predict(self, x):
        for i in range(1, self.niter):
            print(i)
            obs = x[np.random.choice(x.shape[0], 1)][0]
            self.forward(obs, i)
...
if __name__ == '__main__':
    x = torch.randn((25, 100), device=torch.device("cuda"))

    som = SOM(100, 100, 100, 100)
    som.fit_predict(x)
